...
setwd("~/Desktop/calab_data/code_bases/Skmer-2/final_figures/child-parent/")
...
error_percent_plot_p = ggplot(data = all[(
  ...
  scale_y_continuous(breaks = pretty(c(-4,1.5), n = 5)) +
  scale_x_log10(breaks = c(0.001,0.002, 0.004, 0.006, 0.008, 0.01, 0.02), 
...
ggsave("./error_percent-NEWDIST.pdf", error_percent_plot_p, width = 12, height = 15)
  
##########################################################
# FIGURE S2                                              #
##########################################################

error_nolog_plot_p = ggplot(data = all[(
  #species_l %in% c("moss", "rotifer") &
  version %in% c("skmer2+assembly", "skmer1", "skmer2+respect") & 
    variant %in% c("err6+new", "r50-err6+new") & 
    as.numeric(true_distance) > 0
  ...
  variant == "def" & 
    version == "skmer1" & 
    as.numeric(true_distance) > 0)], 
aes(y = ( as.numeric(estimated_distance) - as.numeric(true_distance) ), 
    x = as.numeric(true_distance), 
    color = version)) +
  facet_grid(species_l~coverage) +
  #geom_hline(yintercept = -4, linetype = "dotted", color = "red") +
  ...
  guides(color=guide_legend(title="version", size=20)) +
  xlab("true genomic distance (%)") + 
  ylab("estimated distance - true distance") + 
  scale_color_manual(values = c("#d95f02", "#1b9e77", "#7570b3")) +
  scale_fill_manual(values = c("#d95f02", "#1b9e77", "#7570b3")) 

error_nolog_plot_p
ggsave("./error_nolog.pdf", error_nolog_plot_p, width = 12, height = 15)
...
mean(ifelse( log2(as.numeric(not_all[version=="skmer2+respect", estimated_distance]) / 
               as.numeric(not_all[version=="skmer2+respect", true_distance])) < -4 | 
          is.na(log2(as.numeric(not_all[version=="skmer2+respect", estimated_distance]) 
                     / as.numeric(not_all[version=="skmer2+respect", true_distance]))), 
        -4, 
        log2(as.numeric(not_all[version=="skmer2+respect", estimated_distance]) / 
               as.numeric(not_all[version=="skmer2+respect", true_distance])))) 


mean(ifelse( log2(as.numeric(not_all[version=="skmer2+assembly", estimated_distance]) / 
                    as.numeric(not_all[version=="skmer2+assembly", true_distance])) < -4 | 
               is.na(log2(as.numeric(not_all[version=="skmer2+assembly", estimated_distance]) 
                          / as.numeric(not_all[version=="skmer2+assembly", true_distance]))), 
             -4, 
             log2(as.numeric(not_all[version=="skmer2+assembly", estimated_distance]) / 
                    as.numeric(not_all[version=="skmer2+assembly", true_distance])))) 


mean(ifelse( log2(as.numeric(not_all[version=="skmer1", estimated_distance]) / 
                    as.numeric(not_all[version=="skmer1", true_distance])) < -4 | 
               is.na(log2(as.numeric(not_all[version=="skmer1", estimated_distance]) 
                          / as.numeric(not_all[version=="skmer1", true_distance]))), 
             -4, 
             log2(as.numeric(not_all[version=="skmer1", estimated_distance]) / 
                    as.numeric(not_all[version=="skmer1", true_distance])))) 
...
mean(not_all[version=="skmer2+respect" & true_distance == 0.001 & species == "moss", estimated_distance])

mean(not_all[version=="skmer2+assembly" & true_distance == 0.001 & species == "moss", estimated_distance])
